...
class CaptureViewModelTest {
    ...
    @Test fun trackingLossKeepsReturnGuideAndRestartRestoresAlignment() = runTest(dispatcher) {
        ...
        assertTrue(vm.state.value.showReturnGuide)
        assertNull(vm.state.value.trackedFace)
        assertEquals(CaptureGuidance.TRACKING_LOST, vm.state.value.hint)
        val recoveredPrimary = face.copy(x = .55f)
        port.samples.send(TrackingObservation(1, 2200, recoveredPrimary))
        runCurrent()
        assertEquals(CaptureGuidance.FOLLOWING, vm.state.value.hint)
        ...
        runCurrent()
        assertEquals(CaptureGuidance.MOVE_RIGHT, vm.state.value.hint)
        ...
        assertTrue(vm.state.value.showPositioningMask)
        assertFalse(vm.state.value.showReturnGuide)
        assertEquals(CaptureGuidance.PLACE_FACE, vm.state.value.hint)
        assertNull(vm.state.value.trackedFace)
